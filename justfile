...
build_pg PG_VERSION:
	...
	docker build .  -t myprod/db:pg-{{PG_VERSION}} --rm --target=postgres --build-arg PG_VERSION={{PG_VERSION}}
...
build_timescaledb PG_VERSION:
	...
	docker build .  -t myprod/db:ts-{{PG_VERSION}} --rm --target=timescale --build-arg PG_VERSION={{PG_VERSION}}
